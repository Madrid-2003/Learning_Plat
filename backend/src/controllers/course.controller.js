...
const enrolledcourseSTD = asyncHandler(async(req,res)=>{
  ...
  // Ensure the logged-in student's ID matches the ID in params
  // This check is important for security
  if(stdID != req.Student._id){
    throw new ApiError(400, "params and logged student id doesnt match");
  }

  const enrolledCourses = await course.find({ enrolledStudent: stdID })
                                      .select("-enrolledStudent -liveClasses -enrolledteacher");

  // If no courses are found, it means the student isn't enrolled in any.
  // This is not an error, just an empty list.
  // The previous check `if (!Student)` might have been problematic if `find` returned an empty array.
  if (!enrolledCourses || enrolledCourses.length === 0) {
      return res
      .status(200) // Return 200 OK with an empty array or a specific message
      .json(new ApiResponse(200, [], "Student is not enrolled in any courses"));
  }

  return res
  .status(200)
  .json( new ApiResponse(200, enrolledCourses, "Student and enrolled courses fetched successfully"));
});
...
const deleteClass = asyncHandler(async (req, res) => {
        ...
        if (!courseId || !teacherId || !date || !timing) {
            throw new ApiError(400, "Course ID, Teacher ID, Date, and Timing are required to delete a class.");
        }

        if (req.teacher._id.toString() !== teacherId) {
            throw new ApiError(403, "Unauthorized to delete classes from this course.");
        }

        const classDateObject = new Date(date);
        ...
        if (!updatedCourse) {
            throw new ApiError(404, "Course not found or class not found in course.");
        }

        return res
            .status(200)
            .json(new ApiResponse(200, updatedCourse, "Class deleted successfully!"));
    });

export {getCourse, getcourseTeacher, addCourseTeacher, addCourseStudent, enrolledcourseSTD, enrolledcourseTeacher, addClass, stdEnrolledCoursesClasses, teacherEnrolledCoursesClasses, canStudentEnroll,deleteClass} 
