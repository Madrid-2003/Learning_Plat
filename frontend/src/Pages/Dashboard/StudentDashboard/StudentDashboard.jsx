...
function StudentDashboard() {
  ...
  const Handlelogout = async() =>{
    const response = await fetch(`https://learning-plat-36hz.vercel.app/student/logout`, {
      method: 'POST',
      credentials: "include",
      headers: {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`https://learning-plat-36hz.vercel.app/Student/StudentDocument/${ID}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          }
